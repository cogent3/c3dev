...
class VersionUpdater(object):
    ...
    def __init__(self, rootdir=None, version=None,
...
@click.command()
@click.option('-d', '--rootdir', default='', type=click.Path(exists=True),
              help='root directory for project')
@click.option('-v', '--version', required=True, default='',
              help='new version')  # todo allow auto date based version?
...
def main(rootdir, version, version_short, is_release, verbose, mock_run):
    ...
    updater = VersionUpdater(rootdir=rootdir,
                             version=version,
